/**
     ...
     *  author  :   Prodip Datta
     *  handle  :   prodipdatta7
     *  created :   Tuesday 11-August, 2020  05:12:35 PM
**/
...
vector < vector < int > > g ;
vector < vector < pii > > depth ;
int a[N], mx = 0 ;

void dfs(int s, int d, int p = 0) {
	depth[d].emplace_back(a[s], s) ;
	mx = max(mx, d) ;
	for (int i : g[s]) {
		if (i == p)continue ;
		dfs(i, d + 1, s) ;
...
int main() {
	...
	while (test--) {
		...
		dfs(1, 0) ;
		int cnt = 6442 ; 
		//error(mx, sz(depth[mx]), '\n')
		for (int i = mx ; i >= 0 and cnt > 0 ; --i) {
			sort(all(depth[i])) ;
			int k = sz(depth[i]) ;
