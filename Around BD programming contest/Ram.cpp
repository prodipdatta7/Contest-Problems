/**
     ...
     *  author  :   Prodip Datta
     *  handle  :   prodipdatta7
     *  created :   Wednesday 12-August, 2020  11:58:25 AM
**/
...
ll query(int i, int b, int e, int l, int r, ll X) {
	...
	if (l <= b and e <= r) {
		ll cur = 0 ;
		int t = sz(T[i]) ;
		int i1 = upper_bound(all(T[i]), X) - T[i].begin() ;
		int i2 = lower_bound(all(T[i]), X) - T[i].begin() ;
		if (i1 < t) {
			int h = t - i1 ;
			--i1 ;
			cur += ((S[i][t - 1] - (i1 < 0 ? 0 : S[i][i1])) - (X * h)) ;
		}
		if (i2 > 0) {
			cur += ((X * i2) - (S[i][i2 - 1])) ;
		}
	...
	int mid = b + e >> 1 ;
	return query(i << 1, b, mid, l, r, X) + query(i << 1 | 1, mid + 1, e, l, r, X) ;
}
