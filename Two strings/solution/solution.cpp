/**
     ...
     *
     *  created : Friday 15-January, 2021  11:37:42 PM
**/
...
int opList[N][11], n, C, initialCost ;
vector <int>  A, B ;
ll dp[N][N * 10] ;
char finalString[N] ;
...
int main() {
	...
	while (test--) {
		...
		for (int i = 1 ; i <= n ; ++i) {
			...
			B[i] = (int)(ch - 'a') ;
			opList[i][B[i]] = 0 ;
			initialCost += abs(A[i] - B[i]) ;
		}
		int m ; Int(m) ;
		int id, cost ;
		char ch ;
		for (int i = 1 ; i <= m ; ++i) {
			scanf("%d %c %d", &id, &ch, &cost) ;
			++id ;
			int originalDiff = abs(A[id] - B[id]) ;
			int curValue = (int)(ch - 'a') ;
			int possibleDiff = abs(A[id] - curValue) ;
			if (possibleDiff <= originalDiff) {
				cmin(opList[id][curValue], cost) ;
			}
			// cmin(opList[id][curValue], cost) ; 
		...
		for (int i = n ; i >= 1 ; --i) {
			for (int j = 0 ; j <= n * 10 ; ++j) {
				...
				for (int k = 0 ; k < 10 ; ++k) {
					int diff = abs(A[i] - k) ;
					if (diff > j)continue ;
					cmin(costSoFar, dp[i + 1][j - diff] + opList[i][k]) ;
					error(i, j, k, costSoFar)
				}
				dp[i][j] = costSoFar;
			}
		}
		...
		if (res <= initialCost) {
			for (int i = 1 ; i <= n ; ++i) {
				bool got = false ; 
				for (int j = 0 ; j <= n * 10 and !got ; ++j) {
					for (int k = 0 ; k < 10 ; ++k) {
						int diff = abs(A[i] - k) ;
						if (diff > j)continue ;
						if (C == dp[i + 1][j - diff] + opList[i][k]) {
							finalString[i - 1] = (char)(k + 'a') ;
							C -= opList[i][k];
							got = true ; 
							break ; 
						}
