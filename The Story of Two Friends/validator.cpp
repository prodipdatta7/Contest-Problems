...
int main() {
	...
	int test = 1 , tc = 0 ;
	Int(test) ;
	if (test < 0)puts("Test case exceeds the lower bound") ;
	else if (test > 100000)puts("Test case exceeds the upper bound") ;
	else puts("Test case limits are OK") ; 
	tc = test ; 
	while (test--) {
		int n, a, b, c ; Int(n, a, b, c) ;
		int ok = 0 ; 
		if (n < 0)puts("N exceeds the lower bound") ;
		else if (n > 1000000000)puts("N exceeds the upper bound") ;
		else ++ok ; 
		if (a < 0)puts("A exceeds the lower bound") ;
		else if (a > 1000000)puts("A exceeds the upper bound") ;
		else ++ok ; 
		if (b < 0)puts("B exceeds the lower bound") ;
		else if (b > 1000000)puts("B exceeds the upper bound") ;
		else ++ok ; 
		if (c < 0)puts("C exceeds the lower bound") ;
		else if (c > 1000000)puts("C exceeds the upper bound") ;
		else ++ok ; 
		if(ok == 4)--tc ; 
	}
	if(tc == 0)puts("Everything is OK") ; 
