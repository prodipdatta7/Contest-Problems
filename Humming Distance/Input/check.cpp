 ...
 *
 *  author  :   prodipdatta7
 *  created :   Friday 29-November, 2019  10:52:12 AM
**/
...
ll solution(int n, int m, string s, string T) {
    if (m > n)return 0 ;
    ll res = 0 ;
    for (int i = 0 ; i <= n ; ++i) {
        for (int j = 0 ; j < 26 ; ++j)
...
int main() {
    ...
    while (test--) {
        ...
        for(int i = 0 ; i < m ; ++i){
            assert(t[i] >= 'a' and t[i] <= 'z') ;
        }
        cout << solution(n, m, s, t) << '\n' ;
    }
    return 0 ;
}