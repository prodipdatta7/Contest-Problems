 ...
 *
 *  author  :   prodipdatta7
 *  created :   Sunday 03-May, 2020  11:14:40 PM
**/
...
#define     fread       freopen("test4.txt","r",stdin)
...
struct LIS_SegTree {
	...
	pii merge(pii& a, pii& b){
		if(a.ff == b.ff){
			if(a.ff == 0)return {a.ff, 1} ; 
			return make_pair(a.ff, add(a.ss, b.ss, mod)) ;
		}
		else {
	...
	void update(int i, int b, int e, int p, pii v){
		if(b == e){
			T[i] = v  ;
			return ; 
		}
		int mid = (b + e) >> 1 ;
		if(p <= mid)update(i << 1, b, mid, p, v) ; 
		else update(i << 1 | 1, mid + 1, e, p, v)  ;
		T[i] = merge(T[i << 1], T[i << 1 | 1]) ;
	}

	pii ask(int i, int b, int e, int l, int r){
		if(l > e or r < b)return make_pair(0, 1) ; 
		if(l <= b and e <= r)return T[i] ; 
		int mid = (b + e) >> 1 ; 
		pii x = ask(i << 1, b, mid, l, r) ;
		pii y = ask(i << 1 | 1, mid + 1, e, l, r) ;
		return merge(x, y) ;
	}

	void LIS(vi& a, int n){
		for(int i = 0 ; i < n ; ++i){
			pii p = ask(1, 1, n, 1, a[i] - 1) ; 
			pii x = ask(1, 1, n, a[i], a[i]) ; 
			p.ff++ ;
			if(x.ff == p.ff){
				x.ss = add(x.ss, p.ss, mod) ; 
				update(1, 1, n, a[i], x) ; 
			}
			else {
				update(1, 1, n, a[i], p) ; 
